...
namespace LibDevicesManager
{
    ...
    public class DS360Setting
    {
        ...
        public static string CorrectErroInPhysicalGenerator(string value)
        {
            double.TryParse(value.Replace(".", ","), out double num);// поставить универсальный разделитель вместо точки и запятой
            int charCount = value.Length;
            int powNum = (int)Math.Ceiling(Math.Log10(num));
            int decCharCount = 4;
            if (powNum < -2)
            {
                decCharCount = 6 + powNum;
            }
            num = MetrologyRound.GetRounded(num, (uint)decCharCount);
            value = Convert.ToDecimal(num).ToString();
            if (errorList.Contains(num))
            {
                
                if ((charCount + powNum) < 6 && charCount<8)
                {
        ...
        private static string CorrectCharlCount(string value, double num)
        {
            ...
            if ((charCount + powNum) < 5 && charCount < 8)
            {
